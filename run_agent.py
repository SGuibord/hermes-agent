    ...
    def run_conversation(
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            try:
                ...
                # Strip JSON-wrapped text: some models output plain text wrapped in JSON
                # ({"content":"..."}, {"text":"..."}, {"action":"text","action_input":"..."}).
                # This is display-only — never dispatches tool calls.
                # Only applies when the model returned no real tool_calls.
                if (
                    ...
                    if _c.startswith("{") and _c.endswith("}"):
                        try:
                            ...
                            if isinstance(_parsed, dict):
                                ...
                                # ReAct text-only action ({"action":"text","action_input":"msg"})
                                if not _text and _parsed.get("action", "").lower() in (
                                    "text", "final answer", "final_answer"
                                ):
                                    _ai = _parsed.get("action_input", "")
